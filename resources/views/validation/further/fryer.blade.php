...
<div class="container-xxl flex-grow-1 container-p-y p-0" style="border-radius: 16px;">
    <div class="card px-5 py-5" style="border-radius: 1rem;">
        <h5 class="card-header d-flex justify-content-between align-items-center p-0 mb-4">
            Fryer {{$asset->detail}}
            <a href="{{ route('validation.addDataFryer', $asset->uuid) }}" class="btn btn-primary">
                +
            </a>
        </h5>
        ...
        <div class="table-responsive text-nowrap">
            <table class="table table-bordered text-center align-middle">
                ...
                <tbody>
                    @forelse ($dataFryer as $item)
                    <tr>
                        <th>{{ $loop->iteration }}</th>
                        <td>{{ $item->start_testing}}</td>
                        <td>{{ $item->product_name }}</td>
                        <td>{{ $item->machine_name_2 }}</td>
                        <td>{{ $item->machine_brand_2 }}</td>
                        <td>{{ $item->machine_type_2 }}</td>
                        <td>{{ $item->machine_speed_conv_2 }}</td>
                        <td>{{ $item->machine_capacity_2 }}</td>
                        <td class="d-flex justify-content-center">
                            <a href="{{ route('validation.printPDFFryer', $item->id) }}" class="btn btn-primary btn-sm me-2" target="_blank">
                                Cetak PDF
                            </a>
                            <form action="{{ route('validation.deleteFryerData', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                @csrf
                                @method('DELETE')
