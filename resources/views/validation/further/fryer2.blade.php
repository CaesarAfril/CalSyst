                    ...
                    @forelse ($dataFryer2 as $item)
                        <tr>
                            ...
                            <td class="d-flex justify-content-center">
                                <a href="{{ route('report.fryer2.print', $item->id) }}" class="btn btn-primary btn-sm me-2" target="_blank">
                                    Cetak PDF
                                </a>
                                <form action="{{ route('validation.fryer2.delete', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
