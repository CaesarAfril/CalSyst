    ...
    <div class="card px-5 py-5" style="border-radius: 1rem;">
        ...
        <div class="table-responsive text-nowrap">
            <table class="table table-bordered text-center align-middle">
                ...
                <tbody>
                    @forelse ($dataFryerMarel as $item)
                        <tr>
                            ...
                            <td class="d-flex justify-content-center">
                                <a href="{{ route('report.fryerMarel.print', $item->id) }}" class="btn btn-primary btn-sm me-2" target="_blank">
                                    Cetak PDF
                                </a>
                                <form action="{{ route('validation.fryerMarel.delete', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
