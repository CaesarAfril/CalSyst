...
<div class="container-xxl flex-grow-1 container-p-y px-0">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('validation.storehiCook', $asset->uuid) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row mb-3">
                    <div class="col-sm-6 mb-3">
                        <label for="hi_cook_product_id" class="form-label">Nama Produk</label>
                        <select name="hi_cook_product_id" id="hi_cook_product_id" class="form-control" required>
                    ...
                    <div class="col-sm-6 mb-3">
                        <label for="target_temperaturea" class="form-label">Target suhu inti produk</label>
                        <input type="text" name="target_temperaturea" id="target_temperaturea" class="form-control" placeholder="Masukkan target suhu inti produk" required>
                    </div>
                </div>
...
@section('script')
<script>
    document.getElementById('hi_cook_product_id').addEventListener('change', function() {
        const selectedOption = this.options[this.selectedIndex];
        const min = selectedOption.getAttribute('data-min');
        const max = selectedOption.getAttribute('data-max');
        const suhuInput = document.getElementById('setting_machine_temperature');
